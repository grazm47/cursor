...
def get_response_to_question(self, question):
    """
    Получение ответа на вопрос пользователя (стиль Добрыни)
    """
    ...
    if any(word in question_lower for word in ['доставка', 'доставить', 'курьер', 'почта', 'транспорт']):
        return "Доставка по всей России! 🚚 Курьерская доставка и транспортные компании. Стоимость зависит от адреса и объема. Оставьте контакты для расчета! 📦"
    
    # Скидки
    if any(word in question_lower for word in ['скидка', 'акция', 'дешевле', 'выгодно']):
        return "Скидки есть! 🎉 От 5 рулонов - скидка 5%, от 10 рулонов - 10%. Бесплатная доставка от 10000₽. Оставьте контакты - предложим лучшие условия! 💰"
