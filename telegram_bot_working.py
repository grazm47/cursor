...
class DobrynyaWorkingBot:
    ...
    def get_ai_response(self, user_id, message):
        """Получение ответа от GigaChat AI или локальной базы знаний"""
        # Сначала пытаемся использовать AI
        if self.ai_available:
            try:
                access_token = self._get_access_token()
                if access_token:
                    return self._call_gigachat_api(user_id, message)
            except Exception as e:
                print(f"❌ Ошибка AI, переключаемся на локальную базу: {e}")
        
        # Если AI недоступен, используем локальную базу знаний
        return self.get_smart_response(message)

    def _call_gigachat_api(self, user_id, message):
        """Вызов GigaChat API"""
        try:
            access_token = self._get_access_token()
            if not access_token:
...
dobrynya_bot = DobrynyaWorkingBot()
